...
("yyyy-MM-dd").parse(model.getDataFim());
			...
			modelAndView.addObject("total_contas_receber", 
contaRepository.sumByUsuarioAndData
(usuarioDto.getIdUsuario(), dataIni, dataFim, 1));

			modelAndView.addObject("total_contas_pagar", 
contaRepository.sumByUsuarioAndData
(usuarioDto.getIdUsuario(), dataIni, dataFim, 2));
		}
		catch(Exception e) {
			modelAndView.addObject
("mensagem", "Falha ao carregar dashboard: " + e.getMessage());
		}
		
		return modelAndView;
	}
	
	@RequestMapping(value = "/admin/filtrar-dashboard", 
method = RequestMethod.POST)
	public ModelAndView filtrarDashboard(ConsultaContasModel model, 
